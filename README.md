...
> [!NOTE]  
> 本项目为将闲置算力(GPU)通过MaaS方式在互联网上共享，同时获得互联网上其他用户调用分享的模型API费用收益
> [!IMPORTANT]
...
1. 本地大模型一键接入star-fire云服务，向所有注册用户提供openAI api标准的大模型API服务
2. 支持推理引擎、模型自动发现，实时注册，支持代理模式，转发模型服务
...
11. 支持tools调用
12. 支持自定义价格（上下限），平台可设置客户端能设置的价格上下限
13. 支持命令行客户端通过配置文件为每个模型单独设置价格
